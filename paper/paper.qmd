...
The CFPS records survey data by directly sending agents to the homes of respondents, aiding respondents in the process of filling out a comprehensive survey about their families and themselves [@cfps]. The agents closely interact with the respondents as they complete the survey, ensuring that the recorded information is complete and appropriate. For example, the agent may require the respondent to input an answer for certain questions, even if the initial response of the respondent is "I don't know" [@cfps_questions]. Moreover, the majority of recorded variables in the CFPS surveys are categorical, and they may have ambiguous boundaries.

A portion of surveys were conducted through phone calls, utilizing the same procedure as physical interview sessions. All interviews were facilitated by computer assisted personal interviewing (CAPI) technology provided by the Survey Research Center (SRC) at the University of Michigan [@cfps_measure]. This system helps the researchers design complex interviews tailored to different members of the household while reducing measurement error and monitoring the quality of interviews. 

In the case of young children considered incapable of completing full surveys, their parents provided responses for them [@cfps_questions]. This naturally leads to biases, but it is not relevant to this analysis because only data on the working population was used.

The direct interactions between respondents and agents may effectively maximize the rate of completion of CFPS surveys, but the forced responses might not accurately reflect the true opinions of the respondents, and the fact that an agent directly observes respondents as they fill out the survey may encourage them to conceal information or select answer choices that are considered ordinary. Disparities may be introduced by phone interviews, as phone calls have different dynamics than face-to-face interactions.
...
ggplot(all_combined, aes(x = ln_hourly_wage)) +
  ...
  annotate(
    "text",
    x = mean(all_combined$ln_hourly_wage, na.rm = TRUE) - 3,
...
ggplot(all_combined, aes(x = years_of_education)) +
  ...
  annotate(
    "text",
    x = mean(all_combined$years_of_education, na.rm = TRUE) + 3, # Position text slightly to the right of the mean line
    y = Inf, # Position text at the top of the plot
...
ggplot(all_combined, aes(x = pot_work_years)) +
  ...
  annotate(
    "text",
    x = mean(all_combined$pot_work_years, na.rm = TRUE) + 8, # Position text slightly to the right of the mean line
    y = Inf, # Position text at the top of the plot
...
# Survey Methodology Overview and Evaluation {#sec-method}

CFPS adopts a stratified, multistage probability-proportional-to-size (PPS) sampling approach to ensure national representativeness. The target sample consists of 16,000 households across 25 provinces and regions, effectively representing approximately 95% of China's population [@cfps_manual]. Sampling is conducted in three stages: primary sampling units such as counties, second-stage sampling units such as villages and urban neighborhoods, and third-stage sampling units that are systematically selected households. 

As previously mentioned in the measurement section of @sec-data, the survey employs a series of detailed questionnaires tailored for different individuals, suited for various levels of analysis [@cfps_measure]. These include: the Community Questionnaire which captures data on infrastructure, demographic trends, economy, and governance within sampled communities; the Household Screening Questionnaire that identifies eligible households and their basic characteristics; the Family Roster Questionnaire that maps relationships and collects demographic and occupational information for all household members, including individuals living apart; the Individual Questionnaires: Separate modules for adults and children, addressing education, health, income, employment, and family dynamics [@cfps_manual]. 

As its name (China Family Panel Studies) suggests, CFPS follows a panel study structure, tracking all "gene members" (baseline individuals and their biological or adopted descendants) over multiple iterations, conducted bi-annually. Changes in family composition due to births, deaths, or household splits are carefully updated through family roster questionnaires in each wave [@cfps_manual]. This longitudinal approach enables the study of dynamic processes such as migration, education, and inter-generational relationships.

To ensure the quality of the data collected, CFPS adopts pre-field mapping, interviewer training, and data cross-verification [@cfps_manual].

Due to its innovative and robust approach, CFPS established itself as the most prominent academic data source for longitudinal social science studies. However, there still exists multiple aspects of imperfection other than those discussed in the main paper.

First, the CFPS excludes certain regions of China due to logistical constraints, such as Xinjiang, Tibet, Inner Mongolia, and Hainan, which limits its national representativeness. These regions have unique cultural, economic, and social dynamics that are underrepresented in the dataset [@cfps_manual]. 

Second, following "gene members" over multiple waves presents unique challenges. Migrant populations, in particular, are difficult to track, and while CATI (Computer-Assisted Telephone Interviewing) helps, migration still led to non-response or partial data collection in subsequent waves, affecting longitudinal data integrity. 

Third, the detailed nature of the questionnaires, especially in later waves, makes it impossible for respondents to complete them in a short sitting. This increases the likelihood of respondent fatigue, leading to incomplete or hurried responses. A study found that an additional hour of survey time increases the probability that a respondent skips a question by 10%–64%, and the total monetary value of aggregated categories such as assets or expenditures declines as the survey proceeds [@JEONG2023102992].

As I looked into the specific questionnaire documents, I found that all CFPS iterations collect data on family-related variables much before personal data [@cfps_questions_2012; @cfps_questions]. This may explain the disparities between the early 2010 and 2012 data and other iterations. Early iterations of CFPS do not record individuals' personal income as a single variable; income is recorded as a sub-component of the different jobs an individual worked [@cfps_questions_2012], and I calculated total personal income from summing the individual values of each job for the purpose of this study. Given that data on jobs were recorded in later stages of the questionnaire, there may be systematic under-reporting of job income due to respondent fatigue. I suspect that the disparities in measurement (having personal income variable in some CFPS iterations and not having it in others), in combination with respondent fatigue, caused the changes in measured educational returns throughout the studied decade.
