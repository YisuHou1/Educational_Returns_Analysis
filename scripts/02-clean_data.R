...
# 2020 data
cfps2020person_raw <- read_sas("data/01-raw_data/cfps2020person_202306.sas7bdat", 
                               NULL)
cfps2020famecon <- read_sas("data/01-raw_data/cfps2020famecon_202306.sas7bdat",
                            NULL)


# 2018 data
cfps2018person_raw <- read_sas("data/01-raw_data/cfps2018person_202012.sas7bdat", 
                               NULL)
cfps2018famecon <- read_sas("data/01-raw_data/cfps2018famecon_202101.sas7bdat",
                            NULL)

# 2016 data
cfps2016person_raw <- read_sas("data/01-raw_data/cfps2016adult_201906.sas7bdat", 
                               NULL)
cfps2016famecon <- read_sas("data/01-raw_data/cfps2016famecon_201807.sas7bdat",
                            NULL)

# 2014 data
cfps2014person_raw <- read_sas("data/01-raw_data/cfps2014adult_201906.sas7bdat", 
                               NULL)
cfps2014famecon <- read_sas("data/01-raw_data/cfps2014famecon_201906.sas7bdat",
                            NULL)

# 2012 data
cfps2012person_raw <- read_sas("data/01-raw_data/cfps2012adult_201906.sas7bdat", 
                               NULL)
cfps2012famecon <- read_sas("data/01-raw_data/cfps2012famecon_201906.sas7bdat",
                            NULL)

# 2010 data
cfps2010person_raw <- read_sas("data/01-raw_data/cfps2010adult_202008.sas7bdat", 
                               NULL)
cfps2010famecon <- read_sas("data/01-raw_data/cfps2010famecon_202008.sas7bdat",
                            NULL)
...
# analysis
cfps2020person_noNA <- cfps2020person |> rename(
  gender = QA002,
  income_quartile = fincome1_per_p) |> select(
...
# Have a subset with no missing values
cfps2018person_noNA <- cfps2018person |> rename(
  gender = GENDER,
  income_quartile = fincome1_per_p) |> select(
    ln_hourly_wage, years_of_education, gender, pot_work_years, exp2, city_hukou,
    is_married, is_party, is_east, income_quartile
...
# Have a subset with no missing values
cfps2016person_noNA <- cfps2016person |> rename(
  income_quartile = fincome1_per_p) |> select(
    ln_hourly_wage, years_of_education, gender, pot_work_years, exp2, city_hukou,
    is_married, is_party, is_east, income_quartile
  ) |> drop_na()
...
cfps2014person <- cfps2014person_raw |>
  ...
  select(cfps2014eduy, CFPS2014_AGE, CFPS_GENDER, QG12, QG6, QA301, 
...
# Have a subset with no missing values
cfps2014person_noNA <- cfps2014person |> rename(
  income_quartile = fincome1_per_p,
  years_of_education = cfps2014eduy) |> select(
    ln_hourly_wage, years_of_education, gender, pot_work_years, exp2, city_hukou,
    is_married, is_party, is_east, income_quartile
  ) |> drop_na()
...
cfps2012person_noNA <- cfps2012person_noNA |> rename(
  income_quartile = fincper_p,
  years_of_education = eduy2012) |> select(
    ln_hourly_wage, years_of_education, gender, pot_work_years, exp2, city_hukou,
    is_married, is_party, is_east, income_quartile
  ) |> drop_na()
...
cfps2010person <- cfps2010person_raw |>
  left_join(cfps2010famecon, by = "fid") |> select(
...
# Have a subset with no missing values
cfps2010person_noNA <- cfps2010person |> rename(
  income_quartile = fincper_p,
  years_of_education = cfps2010eduy_best) |> select(
    ln_hourly_wage, years_of_education, gender, pot_work_years, exp2, city_hukou,
    is_married, is_party, is_east, income_quartile
  ) |> drop_na()
